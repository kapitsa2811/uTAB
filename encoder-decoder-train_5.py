...
aim of code is to insert cropped segmentation and get the predictions
'''
'''

to experiment with sliding window this copy is created
encoder-decoder-train_5.py

exreamly poor results 
'''
...
path_x = path+'/cropText/' #entire text
path_y = path+'/cropSegment/' #segmented text data

total = 0

dump=os.listdir(path_x) # count of entire images
dumpLen=len(dump)
print("\n\t dumpLen1=",dumpLen)
...
print("\n\t text images read=",X_train.shape)
print("\n\t segment images read=",y_train.shape)
...
def createModel1():
    ...
    clf.add(Convolution2D(filters=64, kernel_size=(3, 7), input_shape=(128, 128, 3), padding='same'))
    clf.add(BatchNormalization())
    clf.add(Activation('relu'))
    clf.add(MaxPooling2D(pool_size=(2, 2)))

    clf.add(Convolution2D(filters=128, kernel_size=(3, 7), padding='same'))
    clf.add(BatchNormalization())
    clf.add(Activation('relu'))
    clf.add(MaxPooling2D(pool_size=(2, 2)))

    clf.add(Convolution2D(filters=256, kernel_size=(5, 7), padding='same'))
    clf.add(BatchNormalization())
    clf.add(Activation('relu'))
    clf.add(MaxPooling2D(pool_size=(1, 1)))

    clf.add(Convolution2D(filters=256, kernel_size=(5, 5), padding='same'))
    clf.add(BatchNormalization())
    clf.add(Activation('relu'))
    clf.add(MaxPooling2D(pool_size=(2, 2)))
...
clf.fit(X_train,y_train,batch_size=100, epochs=100,validation_split=0.2,callbacks=callbacks,shuffle=True,verbose=2)
...
'''
    here new crop test is implemented
'''
