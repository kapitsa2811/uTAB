...
path_x1 = path+str(expN0)+'//train/ccBlocks/' #
path_x2 = path+str(expN0)+'//train/wslMaps/' #
path_x3 = path+str(expN0)+'//train//preProcess//' #
...
for indxImg,img in enumerate(sorted(dump)):
    ...
    try:
        ...
        originalIm2 = cv2.imread(path_x2+img)
        originalIm3 = cv2.imread(path_x3+img)

        #print("---->",path_x3+img)
        #print("\n\t originalIm3=",originalIm3.shape)
        ...
        if unableContourMaps==1:
            ...
            delMeTemp2=originalIm2[:,:,0]
            delMeTemp3=originalIm3[:,:,0]
            delMeTemp2=cv2.resize(delMeTemp2, (delMeTemp.shape[1], delMeTemp.shape[0]))
            #print("\n\t shape=",delMeTemp.shape)

            originalIm[:,:,0]=0.3*delMeTemp
            originalIm[:,:,1]=0.3*delMeTemp2
            originalIm[:,:,2]=0.4*delMeTemp3
...
ccTestPaths="/home/kapitsa/PycharmProjects/objectLocalization/DocumentTableSeg/DCEDN/publicationData/8/train/ccBlocksTest//"
ccTestPaths1="/home/kapitsa/PycharmProjects/objectLocalization/DocumentTableSeg/DCEDN/publicationData/8/train/wslMapsTest//"
ccTestPaths2="/home/kapitsa/PycharmProjects/objectLocalization/DocumentTableSeg/DCEDN/publicationData/8/train/preProcessTest//"
...
for pos in range(len(tests)):
    try:
        ...
        temp1=cv2.imread(ccTestPaths+tests[pos])
        temp2=cv2.imread(ccTestPaths1+tests[pos])
        temp3=cv2.imread(ccTestPaths2+tests[pos])
        ...
        if unableContourMaps==1:
            ...
            temp[:,:,0]=0.3*delMeTemp
            temp[:,:,1]=0.3*delMeTemp2
            temp[:,:,2]=0.4*delMeTemp3
...
def createModel2(pretrained_weights=None, input_size=(256, 256, 1)):
    ...
    x = layers.Conv2D(16, kernel_size=(3, 3), padding='same', name='pre_fc')(inputs)  # [B,H,W,512]
    # 沿着宽度方式做rnn
    ...
    # rnn_forward1 = layers.TimeDistributed(layers.GRU(rnn_units, return_sequences=True, kernel_initializer='he_normal'),
    #                                      name='gru_forward1')(rnn_output)
    # rnn_backward1 = layers.TimeDistributed(
    #     layers.GRU(rnn_units, return_sequences=True, kernel_initializer='he_normal', go_backwards=True),
    #     name='gru_backward1')(rnn_output)
    #
    # rnn_output1 = layers.Concatenate(name='gru_concat1')([rnn_forward1, rnn_backward1])  # (B,H,W,256)
    #
    # # conv实现fc Conv achieve fc



    # conv实现fc Conv achieve fc

    fc_output = layers.Conv2D(32, kernel_size=(1, 1), activation='relu', name='fc_output')(
        rnn_output)  # (B,H,W,512)


    conv1 = Convolution2D(16, kernel_size=(3,3), activation='relu', padding='same', kernel_initializer='he_normal')(fc_output)
...
x_val = preprocess_input(X_train)

# define model
model = Unet(BACKBONE, encoder_weights='imagenet')
#model.compile('Adam', loss=bce_jaccard_loss, metrics=[iou_score])
model.compile('Adam', loss=bce_jaccard_loss, metrics=['mae'])
#model.compile(('Adam',loss=bce_jaccard_loss,metric=['mae']))
...
'''
'''
    below import resnet+unet
'''

# from modelArchExp import createModelExp
# clf=createModelExp()
...
clf.fit(X_train,y_train,batch_size=8, epochs=300,validation_split=0.2,callbacks=callbacks,shuffle=True,verbose=2)

modelpath="/home/kapitsa/PycharmProjects/objectLocalization/DocumentTableSeg/model//"
clf.save(modelpath+'///modelCCUnetPreprocess.h5')

#clf.save(modelpath+'///segmentMosel.h5')
